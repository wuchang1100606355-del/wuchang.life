# Dirty Worktree Reconciliation Before Copy Set

Generated at: 2026-06-02T00:45:26+08:00
Expected HEAD: `fbba14b`
Observed HEAD: `fbba14b`
Mode: evidence + classification only
Decision: `PASS_CLASSIFICATION_RECORDED_COPY_SET_RETRY_STILL_REQUIRES_CLEAN_WORKTREE`

## Governance Boundary

- No file contents were read for drift classification.
- No file was deleted, reset, copied, deployed, or used for a DB write.
- No service was restarted.
- Approved listing-only exception: `git_space/scripts_git_bundle_backup.sh`.
- The approved utility remains excluded from any production copy set.
...
## Dirty Summary Before Evidence Files
...
TOTAL=491
 ...
 T=8
??=4
git_diff_stat_exit_code=128
git_diff_summary_exit_code=0
git_ls_files_m_exit_code=0
git_ls_files_d_exit_code=0
git_ls_files_o_exit_code=0
```

The diff commands encountered UNC / symlink compatibility errors while listing metadata. This report preserves those command outputs as evidence and does not treat the partial diff summary as a content review.

## Category Counts

| Category | Count |
| --- | ---: |
| `safe_doc_or_evidence` | 161 |
| `runtime_candidate` | 296 |
| `odoo_candidate` | 17 |
| `generated_noise` | 1 |
| `safe_tool_script_excluded_from_copy_set` | 1 |
| `forbidden_or_needs_human_review` | 2 |
| `deleted_paths` | 1 |
| `typechanged_paths` | 8 |
| `untracked_paths` | 4 |
...
### `safe_tool_script_excluded_from_copy_set`

- ` M` `git_space/scripts_git_bundle_backup.sh`

### `forbidden_or_needs_human_review`
...
- `evidence/server_handoff/dirty_worktree/git_diff_stat_20260602_004526.txt`
...
## Production Copy Set Retry

Retry is not yet authorized. Classification is recorded, but the worktree remains dirty. A separate human-approved remediation step is required before rerunning the production copy-set manifest packet.

## Next Step

`human_review_before_worktree_remediation`
