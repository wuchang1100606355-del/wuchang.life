...
W7TP_SKILLS = (
    ...
    {
        "skill_id": "w7tp_router_wireguard_control",
        "name": "W7TP Router WireGuard Control",
        "version": "1.0.0",
        "source_path": ".skill-build/w7tp-router-wireguard-control/SKILL.md",
        "description": "沿既有 Total Field Merlin 單一路由建立 RT-BE86U WireGuard 雙棧、逐裝置 peer、LAN 直連與路由器 DNS 的 8D ADI 計畫；路由器效果必須另有精確單次 D8。",
        "triggers": [
            "路由器VPN",
            "WireGuard",
            "固定IP",
        ...
        "required_context": "Exact RT-BE86U target, current redacted preimage, one registered router intent, rollback rules, reobservation rules, and separate short-lived single-use D8 authority for any network effect.",
        "validation": "Run the adapter and skill validator. The current tool is plan-only; router/NVRAM/firewall/restart/VPN effects must return HOLD until AUTHORIZE_EXACT_ROUTER_NETWORK_CHANGE is registered and verified.",
        "supporting_source_paths": [
            ".skill-build/w7tp-router-wireguard-control/references/total-field-capability-contract.json",
            ".skill-build/w7tp-router-wireguard-control/scripts/router_total_field_adapter.py",
    ...
    {
        "skill_id": "8d-adi-founder-partner",
        "name": "8D ADI Founder Partner / 小J總場代言候選",
        "version": "1.0.0",
        "source_path": ".skill-build/8d-adi-founder-partner/SKILL.md",
        "description": "讓可替換模型沿既有總場 8D／ADI 索引理解 Founder 發明、讀取進度、定位已登記技能並提供一個最短下一步；不取得總場、D8 或對外代言權威。",
        "triggers": [
            "小J",
            "總場代言",
            "Founder Partner",
        ...
        "required_context": "Current Founder intent, target lock, hash-bound 8DADI memory evidence, current progress, Founder all-skills index, invention/evidence references, and current authority boundary.",
        "validation": "Rebuild and validate the existing Founder skill index, verify all three local function bindings and fail-closed boundaries, and keep progress persistence, external representation, node projection, deployment, activation, ADI writes, and tool effects separately authorized.",
        "supporting_source_paths": [
            ".skill-build/8d-adi-founder-partner/references/founder-invention-contract.md",
            ".skill-build/8d-adi-founder-partner/references/proven-facts.md",
            ".skill-build/8d-adi-founder-partner/references/xiaoj-total-field-binding.md",
            "manifests/ollama_xiaoj_total_field_v0_1/system_prefix.txt",
            "manifests/ollama_xiaoj_total_field_v0_1/root_model_contract.json",
            "tools/total_field_dynamic_context.py",
            "configs/total_field/active_total_field_authority_runtime_v1.json",
        ],
    },
    {
        "skill_id": "8d-adi-state-field-intelligence",
        "name": "8D ADI State Field Intelligence",
        "version": "1.0.0",
        "source_path": ".skill-build/8d-adi-state-field-intelligence/SKILL.md",
        "description": "以有界唯讀盤點與 ADI 段落映射建立狀態場，定位因果、供需、閉環缺口、影響與權威邊界。",
        "triggers": [
            "狀態場",
            "8D ADI 檔案智能",
            "閉環缺口",
        ...
        "status": "READY_LOCAL",
        "tool_refs": [
            "linux:python3 .skill-build/8d-adi-state-field-intelligence/scripts/state_field_inventory.py --root <ROOT>",
            "linux:python3 .skill-build/8d-adi-state-field-intelligence/scripts/adi_map_router.py --root <ROOT> --path <SAFE_RELATIVE_PATH> --allow-content-read",
        ],
        "required_context": "Exact node, root, revision, authority boundary, bounded read budget, and safe pivot paths.",
        "validation": "Run bounded inventory and ADI map checks; writes, archives, commits, pushes, deployments, activation effects, and canonical changes remain separately authorized.",
    },
    {
...
def _formal_skill_contracts(entries: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    founder_partner = entries["8d-adi-founder-partner"]
    assimilator = entries["w7tp-capability-assimilator"]
    transmission = entries["w7tp_generative_transmission"]
    router = entries["w7tp_router_wireguard_control"]
    state_field = entries["8d-adi-state-field-intelligence"]
    research = entries["deep-research"]
    return [
        {
            "id": "8d-adi-founder-partner",
            "skill_execution_state": "READY_LOCAL_READ_ONLY_ADVISORY_CANDIDATE",
            "taxonomy_level": "E2_LOCAL_READ_ONLY_FOUNDER_CONTEXT_AND_PROGRESS_ADVICE",
            "purpose": "作為小J的總場代言候選介面，沿既有 8D／ADI 索引讀取 Founder 意圖、進度、發明證據與已登記技能，提供一個最短有效下一步。",
            "when_to_use": "Founder 詢問發明、已完成成果、目前進度、可用技能、跨模型連續性或下一步工程決策時。",
            "how_to_use": "先由既有動態上下文取得雜湊綁定的目前意圖與證據，再以唯讀進度投影定位剩餘差異，最後只產生建議；不得自行保存進度、代表 Founder 對外發言或執行效果。",
            "triggers": founder_partner["triggers"],
            "context_references": [
                "CURRENT_FOUNDER_INTENT_REF",
                "CURRENT_WORK_TARGET_LOCK_REF",
                "CURRENT_PROGRESS_AND_RECEIPT_REFS",
                "FOUNDER_ALL_SKILLS_INDEX_REF",
                "INDEXED_INVENTION_AND_PROVEN_FACT_REFS",
                "CURRENT_TOTAL_FIELD_AUTHORITY_REF",
            ],
            "input_contract": "FOUNDER_QUERY_PLUS_HASH_BOUND_CURRENT_INTENT_PROGRESS_EVIDENCE_SKILL_AND_AUTHORITY_REFS",
            "output_kind": "XiaoJFounderPartnerAdvisoryCandidate",
            "tool_binding": founder_partner["tool_refs"][0],
            "additional_tool_bindings": founder_partner["tool_refs"][1:],
            "side_effects": False,
            "side_effect_class": "READ_ONLY_CONTEXT_RETRIEVAL_AND_IN_MEMORY_PROGRESS_ADVICE",
            "failure_contract": "MISSING_CURRENT_INTENT_STALE_PROGRESS_CONFLICTING_EVIDENCE_UNAVAILABLE_TOOL_OR_UNKNOWN_AUTHORITY_RETURNS_HOLD_WITH_ONE_SHORTEST_MISSING_PREREQUISITE",
            "d1_d8_projection_ref": "configs/total_field/w7tp_8dadi_d6_contract_v2_3.json#dimensions",
            "d6_mapping": "NOT_APPLICABLE_ADVISORY_CONTEXT_AND_PROGRESS_PROJECTION_ONLY",
            "d8_requirement": "PROGRESS_PERSISTENCE_ADI_WRITE_EXTERNAL_REPRESENTATION_TOOL_EFFECT_NODE_PROJECTION_DEPLOYMENT_ACTIVATION_AND_PROMOTION_REQUIRE_SEPARATE_EXACT_AUTHORIZATION",
            "reobservation_contract": "Rebuild the same dynamic context from indexed sources, verify current intent and progress hashes, and keep advice candidate-only until the affected state is separately executed and reobserved.",
            "source": founder_partner["source_path"],
            "source_sha256": founder_partner["source_sha256"],
            "supporting_sources": founder_partner["supporting_sources"],
            ...
            "progress_persistence_authority": False,
            "node_projection_activated": False,
            "skill_coverage_claim": "CURRENT_INDEX_ONLY_WITH_TRUTHFUL_READINESS_STATE",
            "invention_coverage_claim": "INDEXED_EVIDENCE_ONLY_NOT_ALL_INVENTIONS_PROVEN",
            "single_chain_refs": [
                "tools.total_field_dynamic_context.build_dynamic_context",
                "tools.total_field_dynamic_context.build_total_field_progress_projection",
                "tools.total_field_dynamic_context.build_total_field_correction_contract",
                "manifests/ollama_xiaoj_total_field_v0_1/founder_all_skills_8d_index.json",
                "manifests/ollama_xiaoj_total_field_v0_1/capability_registry.json",
            ],
            "examples": {
                "allow": "從目前索引找回已完成成果、讀取最新進度與技能狀態，直接建議一個最短下一步。",
                "hold": "把聊天記憶當成總場事實、宣稱所有技能都能執行、代替 Founder 對外承諾，或未授權即保存進度、部署、啟用或寫入 ADI。",
            },
        },
        {
            "id": "8d-adi-state-field-intelligence",
            "skill_execution_state": "READY_LOCAL_READ_ONLY_WRITE_EFFECTS_REQUIRE_SEPARATE_AUTHORIZATION",
            "taxonomy_level": "E2_LOCAL_READ_ONLY_STATE_FIELD_ANALYSIS",
            "purpose": "以少量樞紐證據建立檔案、段落、能力、權威、版本與節點的狀態場，定位最小閉環缺口。",
            "when_to_use": "需要有界盤點、ADI 座標映射、供需存依、影響分析、跨節點差異或送達判定時。",
            "how_to_use": "先鎖定節點、根、版本、權威與讀取預算，再執行唯讀 inventory；只有缺口未閉合時才對安全路徑執行有界 ADI map。",
            "triggers": state_field["triggers"],
            "context_references": [
                "CURRENT_FOUNDER_INTENT_REF",
                "CURRENT_NODE_ROOT_REVISION_REF",
                "CURRENT_CANONICAL_AND_AUTHORITY_REFS",
                "BOUNDED_EVIDENCE_AND_READ_BUDGET_REFS",
            ],
            "input_contract": "EXACT_NODE_ROOT_REVISION_AUTHORITY_READ_BUDGET_AND_SAFE_PIVOT_PATHS",
            "output_kind": "StateFieldObservationCandidate",
            "tool_binding": state_field["tool_refs"][0],
            "additional_tool_bindings": state_field["tool_refs"][1:],
            "side_effects": False,
            "side_effect_class": "READ_ONLY_PATH_METADATA_INVENTORY_AND_BOUNDED_SEGMENT_MAPPING",
            "failure_contract": "UNKNOWN_ROOT_REVISION_AUTHORITY_OR_UNSAFE_PATH_RETURNS_HOLD_WITH_NO_WRITE",
            "d6_mapping": "NOT_APPLICABLE_UNLESS_TARGET_BASE_DELTA_REFERENCES_COORDINATES_RECONSTRUCTION_AND_VERIFICATION_CONTRACT_IS_COMPLETE",
            "d8_requirement": "ARCHIVE_WRITE_COMMIT_PUSH_DEPLOY_ACTIVATION_EFFECT_AND_CANONICAL_MUTATION_REQUIRE_SEPARATE_EXACT_AUTHORIZATION",
            "reobservation_contract": "Recheck node, root, revision and target paths after analysis; any drift returns HOLD_STATE_CHANGED and preserves the prior observation.",
            "source": state_field["source_path"],
            "source_sha256": state_field["source_sha256"],
            "examples": {
                "allow": "唯讀盤點目前根目錄，再對最多八個安全樞紐建立 ADI 段落座標並定位第一個閉環缺口。",
                "hold": "根目錄或版本漂移、權威未知、路徑涉及秘密，或未授權即要求歸檔、提交、推送、部署或改正典。",
            },
        },
        {
        ...
        {
            "id": "w7tp_router_wireguard_control",
            "skill_execution_state": "READY_LOCAL_PLAN_ONLY_D8_REQUIRED_FOR_EFFECT",
            "taxonomy_level": "E2_LOCAL_READ_ONLY_E5_ROUTER_EFFECT_ONLY_AFTER_EXACT_D8",
            "purpose": "由總場把路由器 VPN、雙棧、peer、LAN 直連與路由器 DNS 意圖映射至既有 Merlin 治理鏈。",
            "when_to_use": "需要觀測或調整 RT-BE86U WireGuard、IPv4/IPv6、LAN/DNS、peer 或 VPN 切換時。",
            "how_to_use": "先用本機 adapter 產生 hash-bound 8D 計畫；實際網路效果只接受精確 preimage、單一 mutation、rollback、reobservation 與短時效單次 D8。",
            "triggers": router["triggers"],
            "context_references": [
                "CURRENT_FOUNDER_INTENT_REF",
                "CURRENT_ROUTER_REDACTED_PREIMAGE_REF",
                "EXACT_ROUTER_TARGET_AND_MUTATION_REF",
                "ROLLBACK_AND_REOBSERVATION_RULES_REF",
            ],
            "input_contract": "ONE_REGISTERED_ROUTER_INTENT_PLUS_EXACT_TARGET_PREIMAGE_ROLLBACK_AND_REOBSERVATION",
            "output_kind": "RouterAdjustmentStateFieldPacketCandidate",
            "tool_binding": router["tool_refs"][0],
            "side_effects": False,
            "side_effect_class": "PLAN_ONLY_ROUTER_EFFECT_REQUIRES_AUTHORIZE_EXACT_ROUTER_NETWORK_CHANGE",
            "failure_contract": "MISSING_PREIMAGE_ROLLBACK_REOBSERVATION_OR_REGISTERED_D8_SCOPE_RETURNS_HOLD_WITH_NO_NETWORK_EFFECT",
            "d6_mapping": "NOT_APPLICABLE_NETWORK_TRANSPORT_ONLY",
            "d8_requirement": "AUTHORIZE_EXACT_ROUTER_NETWORK_CHANGE_SINGLE_USE_SHORT_LIVED_SCOPE_REQUIRED_FOR_EACH_EFFECT",
            "reobservation_contract": "Verify WireGuard listener, external IPv4 and IPv6, router DNS, LAN access, direct non-transit LAN paths, and measured throughput before declaring PASS or cutting over Tailscale.",
            "source": router["source_path"],
            "source_sha256": router["source_sha256"],
            "supporting_sources": router["supporting_sources"],
            "existing_chain_only": [
                "runtime/router/merlin_intent_driver.py",
                "runtime/router/merlin_apply_queue.py",
                "runtime/router/merlin_approval_gate.py",
            ...
            "examples": {
                "allow": "產生 WireGuard 雙棧或單一 peer 的精確計畫封包並等待 D8。",
                "hold": "模型自行改 NVRAM、停用備援 VPN、輸出 private key，或讓 02/03/04 經另一節點轉送。",
            },
        },
        {
            "id": "deep-research",
...
def _refresh_source_manifest() -> None:
    ...
    for path in (
        ...
        if path not in existing_paths:
            existing_paths.append(path)
    for skill_root in (
        ROOT / ".skill-build/8d-adi-founder-partner",
        ROOT / ".skill-build/8d-adi-state-field-intelligence",
        ...
        ROOT / ".skill-build/w7tp-router-wireguard-control",
    ):
...
def build() -> dict[str, Any]:
    ...
    registry["skill_registration_governance"] = {
        ...
        "registered_skill_ids": [
            "8d-adi-founder-partner",
            "8d-adi-state-field-intelligence",
            "w7tp-capability-assimilator",
            "w7tp_generative_transmission",
            "w7tp_router_wireguard_control",
            "deep-research",
        ],
        "merged_aliases": {
