...
from runtime_adapters.intent_flow_cache_policy import cache_health, evaluate_intent_flow_cache  # noqa: E402
...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "audit_runtime": "ok",
        "continuity_cache": "ok",
        "intent_flow_cache": cache_state["intent_flow_cache"],
        "intent_flow_cache_policy": cache_state["policy_status"],
        "raw_plaintext_cache_allowed": cache_state["raw_plaintext_cache_allowed"],
        "payment_cache_allowed": cache_state["payment_cache_allowed"],
    }


def route(packet: dict[str, Any]) -> dict[str, Any]:
    validation = validate(packet)
    cache_decision = evaluate_intent_flow_cache(packet)
    ...
    result = {
        ...
        "intent_flow_cache": cache_decision,
    }
    write_audit({
        "event": "tensor_route",
        "target": target,
        "packet_hash": result["packet_hash"],
        "intent_flow_cache_allowed": cache_decision.get("cache_allowed"),
    })
    return result
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/health":
            ...
            return
        if self.path == "/intent/cache/status":
            self.send_json(200, cache_health())
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path == "/tensor/route":
            ...
            return
        if self.path == "/intent/cache/evaluate":
            result = evaluate_intent_flow_cache(body)
            write_audit({
                "event": "intent_flow_cache_evaluate",
                "cache_allowed": result.get("cache_allowed"),
                "packet_hash": packet_hash(body),
            })
            self.send_json(200 if result.get("cache_allowed") else 423, result)
            return
        self.send_json(404, {"ok": False, "error": "not_found"})
