...
DIAG_SCRIPT="$ROOT_DIR/deploy/packages/taiji_formal_tensor_runtime_v0_1_0/COLLECT_RUNTIME_NODE_INFO.sh"
...
if health_probe; then
  echo "health_ok_no_restart"
  exit 0
fi

if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE")" >/dev/null 2>&1; then
  echo "already running: $(cat "$PID_FILE")"
  echo "health_failed_collecting_node_info" >&2
  bash "$DIAG_SCRIPT" >&2 || true
  echo "runtime_unhealthy_no_restart_per_governance" >&2
  exit 1
fi

python3 -m py_compile "$ROOT_DIR/runtime_adapters/taiji_formal_tensor_runtime_v0_1_1_adapter.py"
python3 -m py_compile "$ROOT_DIR/runtime_adapters/intent_flow_cache_policy.py"
python3 -m py_compile "$ROOT_DIR/deploy/packages/taiji_formal_tensor_runtime_v0_1_0/runtime_entry_v0_1_1.py"

if [ ! -f "$TAIJI_INTENT_CACHE_POLICY_PATH" ]; then
  echo "missing intent cache policy: $TAIJI_INTENT_CACHE_POLICY_PATH" >&2
  bash "$DIAG_SCRIPT" >&2 || true
  exit 1
fi
...
if ! python3 - "$TAIJI_BIND_HOST" "$TAIJI_PORT" "$LOG_FILE" <<'PY'
import json
import sys
import time
import urllib.error
...
then
  bash "$DIAG_SCRIPT" >&2 || true
  echo "runtime_started_but_health_failed_no_kill_per_governance" >&2
  exit 1
fi
