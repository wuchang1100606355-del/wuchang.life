...
LLM_PUSH_D6_BLOCKED_KEYS = frozenset(
    {"raw_key", "token", "raw_token", "password", "member_plaintext"}
)
LLM_PUSH_SIDE_EFFECT_KEYS = frozenset(
    {"db_write", "deploy", "restart", "router_write"}
)
D7_NESTED_RAW_KEYS = frozenset(
    {
        "raw_payload",
        "raw_data",
        "blob",
...
class PriorityPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        expected_priority = tuple(
            sorted(DECISIONS, key=DECISION_SEVERITY.__getitem__, reverse=True)
        )
        if self.decision_priority != expected_priority:
    ...
    def decision_rank(self, decision: str) -> int:
        """Return fixed severity where ALLOW < HOLD < BLOCK < QUARANTINE."""

        if decision not in DECISION_SEVERITY:
            ...
            deep_copy_json(
                {
                    ...
                    "canonical_tfs_match": self.canonical_tfs_match,
...
def _contains_enabled_key(
    value: JSONValue, names: frozenset[str]
) -> tuple[str, ...]:
    """Return paths for explicitly enabled side-effect keys."""

    found: list[str] = []

    def enabled(item: JSONValue) -> bool:
        """Return whether one JSON value explicitly enables an operation."""

        if item is True:
            return True
        if isinstance(item, str):
            return item.strip().casefold() in {
                "enabled",
                "requested",
                "true",
                "yes",
    ...
    def visit(item: JSONValue, path: str) -> None:
        """Walk nested JSON and record enabled keys by structural path."""

        if isinstance(item, dict):
            for key, nested in item.items():
                child_path = f"{path}.{key}"
                if key.casefold() in names and enabled(nested):
                    found.append(child_path)
                visit(nested, child_path)
        elif isinstance(item, list):
            for index, nested in enumerate(item):
...
class _ConstraintContext:
    ...
    previous_state: EightFieldState
    proposed_state: EightFieldState
    current_state: EightFieldState
    committed_state: EightFieldState
    event: Event
    ...
    if forbidden_paths or allow_paths:
        return _result(
            ...
            (
                ...
                else "EXTERNAL_AUTHORITY_CLAIM_BLOCKED"
            ),
...
def _evaluate_d6_privacy(
    ...
    if context.get("source_mode") == "LLM_PUSH":
        protected_paths = _contains_key(joined, LLM_PUSH_D6_BLOCKED_KEYS)
        side_effect_paths = _contains_enabled_key(joined, LLM_PUSH_SIDE_EFFECT_KEYS)
        if protected_paths or side_effect_paths:
            return _result(
                constraint,
                "BLOCK",
                ...
                {
                    "blocked_paths": list(
                        sorted(protected_paths + side_effect_paths)
                    )
                },
            )
    ...
    if unsupported:
        return _result(
            ...
            (
                ...
                if runtime.caller_context.get("source_mode") == "LLM_PUSH"
                else "D7_REFERENCE_ONLY_REQUIRED"
            ),
...
def _adjudicate(
    ...
    if not decisions:
        return "ALLOW"
    return max(decisions, key=policy.decision_rank)
...
def compare_tfs_equivalence(
    ...
    canonical_tfs_match = canonical_json(left.to_dict()) == canonical_json(
        right.to_dict()
    )
    ...
    return EquivalenceResult(
        ...
        canonical_tfs_match=canonical_tfs_match,
