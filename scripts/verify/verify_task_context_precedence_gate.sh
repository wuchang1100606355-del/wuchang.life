...
run_self_test() {
  ...
  output="$($script \
    ...
    --old-context-role EVIDENCE_ONLY)"
  grep -Fq 'STATE=PASS_CURRENT_TASK_CONTEXT' <<<"$output"
  ...
  output="$($script \
    ...
    --live-state-checked true \
    --old-context-role EVIDENCE_ONLY)"
  grep -Fq 'STATE=HOLD_TASK_CONTEXT_ROLLBACK' <<<"$output"
  ...
  output="$($script \
    ...
    --current-attachment-designated false \
    --live-state-checked true \
    --old-context-role EXECUTION_AUTHORITY)"
  grep -Fq 'STATE=HOLD_TASK_CONTEXT_ROLLBACK' <<<"$output"
...
while (($#)); do
  case "$1" in
    ...
    *) emit HOLD_TASK_CONTEXT_ROLLBACK 'task-context gate received an unknown argument'; exit 0 ;;
  esac
done
...
case "$selected_source" in
  ...
  *) emit HOLD_TASK_CONTEXT_ROLLBACK 'selected context source is missing or invalid'; exit 0 ;;
esac

case "$selected_carrier" in
  INLINE_CURRENT_TURN|CURRENT_TURN_ATTACHMENT|HISTORICAL_ATTACHMENT|PRIOR_RUN|SYSTEM_STATE) ;;
  *) emit HOLD_TASK_CONTEXT_ROLLBACK 'selected context carrier is missing or invalid'; exit 0 ;;
esac

for value in "$current_attachment_designated" "$live_state_checked"; do
  case "$value" in
    true|false) ;;
    *) emit HOLD_TASK_CONTEXT_ROLLBACK 'task-context boolean input is invalid'; exit 0 ;;
  esac
done

case "$old_context_role" in
  EVIDENCE_ONLY|EXECUTION_AUTHORITY) ;;
  *) emit HOLD_TASK_CONTEXT_ROLLBACK 'old context role is missing or invalid'; exit 0 ;;
esac
