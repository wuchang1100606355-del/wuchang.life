...
source_attachment = stance_fixture.get("source_attachment")
if not isinstance(source_attachment, str) or not source_attachment.startswith("attachment://"):
    raise SystemExit("source attachment must remain an opaque attachment reference")
if stance_fixture.get("source_attachment_role") != "HISTORICAL_EVIDENCE_ONLY":
    raise SystemExit("source attachment must remain historical evidence only")
