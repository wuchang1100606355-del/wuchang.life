...
- 接收端 HTTP 409 在回應包含受限安全格式的 `CONFLICT_*`，或明確不可逆的 `HOLD_PACKET_TTL_EXPIRED` reason code 時，寫入 append-only `outbox_terminal` 並停止該 carrier／peer 的無限重試；其他 `HOLD_*`、無法解析或暫時性回應仍維持耐久重試。終止紀錄只是不再重送的 D4 證據，不是成功收據或權威升格。
- `01_NODE_INDEX/discovered` 的拓撲證據同時綁定「被發現節點」與「觀測者節點」；不同來源觀察到同一尾網節點時，各自寫入 `observers/<source_node>`，不得因內容相同而互相覆寫或碰撞。
