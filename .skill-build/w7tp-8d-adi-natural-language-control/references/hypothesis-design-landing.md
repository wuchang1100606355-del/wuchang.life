...
- `READY_TO_LAND`：實作與驗證閉合；若屬可回復工程 AUTO-LAND（自動落地）作用域且權威硬閘已閉合，不得停留，應立即解析本次自然語言意圖封套並前進；需要獨立人類／組織／法律權威者保持 HOLD 至該權威成立。
- `LAND_NOW`：本次意圖涵蓋該可回復工程效果、正式效果權威契約已允許、目標前像、回復與時窗有效時，立即落地，不要求重複確認。
...
`AUTO_LAND_DEFAULT=true` 只適用於本次自然語言明示、已證明可回復且不擴張人類權利的工程效果。個資同意、付款／扣款、正式法律或組織效果、角色／權限／身分升級、帳號或付費變更、對外公開、不可逆或破壞性效果不由 AUTO-LAND 推定授權，必須依各自權威契約閉合。

在上述可回復工程作用域內：
1. 設計閉合後自動進入實作。
2. 實作完成後自動進入驗證。
3. 驗證、回復與 Total Field（總場）正式效果硬閘閉合後自動進入 `LAND_NOW`。
4. 落地後自動 `REOBSERVE`；不符合目標即自動 rollback（回復）。
5. 不因尚未 Canonical（正典）或工作樹有無關髒狀態阻擋本次精確、可回復的工程效果。
6. 若效果超出本次人類意圖封套、需要獨立同意／付款／法律／組織／角色權威、不可逆破壞、必要權限／憑證不可取得，或存在無法消解的互斥目標，必須 `HOLD`。
