...
- A2A cloud organ（A2A 雲端器官）固定使用 isolated task workspace（隔離任務工作區）。pointer handshake（指標握手）與受治理候選階段不得直接讀寫 Taiji_Hub；偵測到未授權 tool-call（工具呼叫）、artifact（產物）或 workspace mutation（工作區變更）即 HOLD。供應商模型內部 system context／cache／reasoning pipeline（系統上下文／快取／推理流水線）屬不可驗證黑盒；W7TP 只宣稱控制外部輸入、地端重構投影、候選回傳與 Total Field 裁決，不把供應商內部狀態納入可信系統。
- Complex-code（複雜程式）任務在 Gemini A2A 已 pointer-first 綁定時，natural-language goal runner（自然語言目標執行器）必須自動以當前 WORK_LEDGER／ACTION_LEDGER／CURRENT_CONVERSATION_CHECKPOINT／Native ADI selected records（工作帳本／動作帳本／當前對話檢查點／原生 ADI 選定紀錄）發行真實 task-state Origin-State Minimum Packet（任務狀態原胞最小封包），先取得 Gemini candidate-only reasoning（僅候選推理），再交 MSI local source builder（MSI 地端來源建構器）獨立讀取 shadow source（隔離來源）與實作；Gemini HOLD 可降級回 MSI local，但 pointer-first 模式不得重新啟用 legacy Vertex direct-context（舊 Vertex 直接上下文）旁路。任何 source delta 仍須 deterministic validation（確定性驗證）與 Total Field sole receiver（總場唯一接收器）ALLOW 後才能 live land（現行落地）。
