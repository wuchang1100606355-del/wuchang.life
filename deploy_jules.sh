...
echo "================================================="
echo "☯️ [Jules 中樞] 接收神諭：創檔、建立結界、執行合併..."
echo "================================================="

# 0. 維和刪檔程序 (貫徹神諭：修正前先刪檔，確保淨空)
if [ -f "jules_core_v21_2.py" ]; then
    echo "⚠️ [維和協議] 偵測到舊有核心檔案，啟動淨空重置程序..."
    rm -f "jules_core_v21_2.py"
    echo "🗑️ [淨空完成] 舊版核心已徹底抹除。"
else
    echo "✨ [淨空確認] 當前目錄無舊版核心干擾。"
fi

# 1. 創檔與寫入存檔 (使用 heredoc 安全寫入)
cat << 'EOF' > jules_core_v21_2.py
# -*- coding: utf-8 -*-
"""
...
echo "✅ [維和確認] jules_core_v21_2.py 核心程式碼已成功創檔並寫入！"

# 2. 啟動虛擬太極結界 (Python venv) - 解決 PEP 668 系統保護問題
if [ ! -d "jules_env" ]; then
    echo "🛡️ [結界展開] 正在建立專屬獨立環境 (jules_env) 以避開系統限制..."
...
if ! python3 -c "import torch" &> /dev/null; then
    echo "⚠️ [依賴警告] 結界內缺乏 PyTorch，正在為您安裝 (此步驟可能需要幾分鐘下載)..."
    pip install torch
fi

echo "🚀 [點火序列] 正在結界內執行合併與啟動指令..."
echo "================================================="

# 4. 執行核心
python3 jules_core_v21_2.py
